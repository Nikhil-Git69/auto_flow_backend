...
router.post("/upload-file", (req, res) => {
    upload.fields([
        ...
        try {
            ...
            const analysisId = `analysis-${(0, uuid_1.v4)()}`;
            const analysisResult = await (0, aiAnalysisService_1.analyzeDocumentWithAI)(documentFile.buffer, documentFile.originalname, documentFile.mimetype, formatType, formatRequirements, templateFile, analysisId);
            ...
            if (analysisResult.processedContent && !documentFile.mimetype.includes('pdf')) {
                ...
                correctedContent = await (0, aiAnalysisService_1.generateCorrectedText)(analysisResult.processedContent, analysisResult.issues);
            }
            ...
            if (documentFile.mimetype.includes('pdf')) {
                ...
                const issuesWithIds = (analysisResult.issues || []).map((issue) => ({
                    ...issue,
                    id: issue.id || (0, uuid_1.v4)(),
                    pageNumber: issue.pageNumber || 1,
                    position: issue.position || { top: 0, left: 0, width: 0, height: 0 }
                }));
                const fixedIssueIds = issuesWithIds.map((val) => val.id); // Fix all
                const pdfBuffer = await (0, aiAnalysisService_1.generateCorrectedPDF)(documentFile.buffer, issuesWithIds, fixedIssueIds);
            ...
            (0, webhookService_1.notifyAnalysisCompleted)({
...
router.post("/export-corrected", async (req, res) => {
    try {
        const { analysisId, fixedIssueIds } = req.body;
        const analysis = await Analysis_1.default.findOne({ analysisId });
        if (!analysis || !analysis.fileData) {
            return res.status(404).json({ success: false, error: "Analysis or original file not found" });
        }
        const correctedDoc = await (0, aiAnalysisService_1.generateCorrectedDocument)(analysis.fileData, analysis.fileName, analysis.issues, fixedIssueIds);
...
router.get("/", async (req, res) => {
    try {
        ...
        const analyses = await Analysis_1.default.find({ userId })
...
router.get("/:id/download", async (req, res) => {
    try {
        ...
        const analysis = await Analysis_1.default.findOne(query).select('+fileData');
        if (!analysis || !analysis.fileData) {
...
router.get("/id/:id", async (req, res) => {
    try {
        ...
        const query = isValidObjectId
            ...
            : { analysisId: analysisId };
        const analysis = await Analysis_1.default.findOne(query);
        if (!analysis)
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const result = await Analysis_1.default.deleteOne(query);
...
router.delete('/', async (req, res) => {
    try {
        ...
        const result = await Analysis_1.default.deleteOne({
            userId,
            fileName,
