    ...
    try {
        ...
        if (existingUser) {
            ...
            if (!existingUser.isEmailVerified) {
                ...
                try {
                    await (0, webhookService_1.notifyEmailVerification)({
...
router.post("/login", async (req, res) => {
    try {
        ...
        await user.save();
        const token = jsonwebtoken_1.default.sign({ userId: user._id, email: user.email, role: user.role, name: user.name, collegeName: user.collegeName }, JWT_SECRET, { expiresIn: '7d' });
        res.json({
            success: true,
...
router.post("/forgot-password", async (req, res) => {
    try {
        ...
        if (user) {
            const otp = (0, otpService_1.generateOtp)(email);
            try {
                await (0, webhookService_1.notifyPasswordReset)({
                    userId: user._id.toString(),
                    userName: user.name,
                    userEmail: user.email,
                    otp,
...
router.post("/verify-reset-otp", async (req, res) => {
    try {
        ...
        if (!email || !otp)
            return res.status(400).json({ success: false, error: "Email and OTP are required" });
        const verificationResult = (0, otpService_1.verifyOtp)(email, otp);
